...
const Home = () => {
  ...
  return (
    <div className="relative w-full h-screen flex flex-col md:flex-row">
      {/* Toggle Button */}
      <div className={`absolute top-16 z-10 transition-all duration-300 ${toggle ? 'left-80' : 'left-4'} md:block hidden`}>
      ...
      {/* Sidebar */}
      <div className={`transition-all duration-300 ${toggle ? 'w-1/4' : 'w-0'} bg-gray-300 overflow-hidden md:w-1/4 md:overflow-visible md:flex md:flex-col md:h-screen ${!toggle ? 'md:w-0' : 'md:w-1/4'} md:block`}>
        {toggle && (
          <div className="p-4 relative">
            ...
            <div className="mt-12">
                <p className="text-2xl">
                    MAITREYA BUDDHA IN GESTURE OF FEARLIESNESS (ABAHYA MUDRA)
                </p>
            </div>
            ...
            <p className="text-[#cd853f]">ReadMore</p>
            </div>
            <div className="space-x-3 mt-2">
                <button className="bg-[#cd853f] px-1 py-1 text-white hover:bg-[#8b4513] hover:pointer rounded-md border border-[#cd853f] hover:scale-100 duration-100">ADD TO COLLECTION</button>
                <button className="p-1 text-[#cd853f] bg-white border border-[#cd853f] rounded-md hover:pointer">SOUVENIR</button>
            </div>
          </div>
        )}
      </div>
